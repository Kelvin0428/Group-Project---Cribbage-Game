...
public class Cribbage extends CardGame {
	...
	public static Properties cribbageProperties;
	...
	*/
	...
	public static final IPlayer[] players = new IPlayer[nPlayers];
	...
	private void deal(Hand pack, Hand[] hands) {
		...
		for (int i = 0; i < nPlayers; i++) {
			...
			logger.logDeal(i, canonical(players[i].hand));
	...
	private void discardToCrib() {
		...
		for (int i = 0; i < nPlayers; i++) {
			...
			for (int j = 0; j < nDiscards; j++) {
				Card c = players[i].discard();
				transfer(c, crib);
				h1.insert(c.getSuit(), c.getRank(), false);
			}
			logger.logDiscard(i,canonical(h1));
			crib.sort(Hand.SortType.POINTPRIORITY, true);
		}
	}

	private void starter(Hand pack, int dealer) {
		...
		Card dealt = randomCard(pack);
		logger.logStarter(canonical(dealt));
		
		dealt.setVerso(false);
		transfer(dealt, starter);
		
		ScoreRule cribRule = RuleFactory.getInstance().getCribRule(starter);
		int points = cribRule.getScore(dealer, scores[dealer], players[dealer].hand);
		if (points > 0) {
			// add points for a Jack starter
			scores[dealer] += points;
			updateScore(dealer);
			//logger.logScore(dealer, scores[dealer], points, "starter", "[JH]");
		}
	}
		...
		while (!(players[0].emptyHand() && players[1].emptyHand())) {
			...
			if (nextCard == null) {
				if (s.go) {
					...
					s.newSegment = true;
					ScoreRule endrule = RuleFactory.getInstance().getRoundEnd(true);
					scores[currentPlayer] += endrule.getScore(currentPlayer,
							scores[currentPlayer],s.segment);
					updateScore(currentPlayer);
					//logger.logScore(currentPlayer, scores[currentPlayer], 1, "go", null);
				} else {
				...
				logger.logPlay(currentPlayer,total(s.segment),canonical(nextCard));

				ScoreRule rule = RuleFactory.getInstance().getPlayRule();
				int points = rule.getScore(currentPlayer,
						scores[currentPlayer],s.segment);
				if (points > 0) {
					scores[currentPlayer] += points;
					updateScore(currentPlayer);
					//logger.logScore(currentPlayer, scores[currentPlayer], points, "rule", "cards");
				}
	...
	void showHandsCrib(List<Integer> handscores, int dealer) {
		// non dealer, dealer, then crib (points go to the dealer)
		for(int i = 0; i < nPlayers; i++){
			scores[i] += handscores.get(i);
			updateScore(i);
			//logger.logShow(i, canonical(starter.getLast()), canonical(hands[i]));
			logger.logHand(i, canonical(starter.getLast()));
			if(i == dealer){
				scores[i] += handscores.get(handscores.size() - 1);
				updateScore(i);
				//logger.logShow(i, canonical(starter.getLast()), canonical(crib));
				logger.logHand(i, canonical(starter.getLast()));
			}
		}
			
	}
	private List<Integer> calcHandsCrib(int dealer){
		List<Integer> scores = new ArrayList<>();
		ScoreRule showrule = RuleFactory.getInstance().getShowRule(starter);
		for(int i=0;i<nPlayers;i++){
			scores.add(showrule.getScore(i,
					getScores()[i],players[i].hand));
		}
		scores.add(showrule.getScore(dealer, getScores()[dealer],crib));
		return scores;
	}

	private void storeHands(){
		for (int i = 0; i<nPlayers; i++){
			logger.storeHand(i, players[i].hand);
		}
	}
	...
	public Cribbage()
	{
		...
		initScore();
		logger.logStart(SEED, cribbageProperties.getProperty("Player0"),cribbageProperties.getProperty("Player1"));
		...
		storeHands();
		List<Integer> handScores = calcHandsCrib(dealer);
		play();
		showHandsCrib(handScores, dealer);
